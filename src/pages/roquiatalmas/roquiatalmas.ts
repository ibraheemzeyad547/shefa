...
/**
 * Generated class for the RoquiatalmasPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-roquiatalmas',
  templateUrl: 'roquiatalmas.html',
})
export class RoquiatalmasPage {
  ...
  settingMusicControl(index){
    ...
    ngOnInit() {
        ...
        this.soundOfArray[1] =  ["0bb303628612d21d9b4963a6e288eb8a.mp3","https://healingofthequran.com/menu/11.mp3","roquiatalmas","رقية المس",'1'];
        this.soundOfArray[2] = ["435033fafd8947a063730e223cc5500e.mp3","https://healingofthequran.com/menu/10.mp3","roquiatalmas","رقية المس",'2'];
    ...
    downloadeRecordingAndroid(index:any){
        ...
        this.translate.get('roquiatalmas').subscribe( value => {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad RoquiatalmasPage');
  }
    ...
    prepareAudioFile(index:any) {
        ...
        this.is_ready[index]=false;
        let transfer = this.transfer.create();
        let result = this.file.createDir(this.storageDirectory, "ShifaALQuran", true).then(res=>{
    ...
    playRecording() {
        this.storage.get('changeSound').then((val) => {
            this.redingType = val;
            this.formpagesProvider.watchList(this.soundOfArray[val][3],'2',50,this.soundOfArray[val][2],this.soundOfArray[val][1]).then(data=>{})
            for(let i=1;i<this.soundOfArray.length;i++){
