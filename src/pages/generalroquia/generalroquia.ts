...
/**
 * Generated class for the GeneralroquiaPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-generalroquia',
  templateUrl: 'generalroquia.html',
})
export class GeneralroquiaPage {
  ...
  settingMusicControl(index){
    ...
    ngOnInit() {
        ...
        if(this.switchShow!=undefined){
            ...
            if(this.selectSegment=="listening"){
        ...
        this.formpagesProvider.watchList("الرقية العامة الشرعية",'1',15,'generalroquia').then(data=>{})
        ...
        this.soundOfArray[1] =  ["587b7fa3025eaa436a26efe8f4bb8a2f.mp3","https://healingofthequran.com/menu/1.mp3","generalroquia","الرقية الشرعية العامة","1"];
        this.soundOfArray[2] =  ["20066f2544f8635a5eba110a6e2cfc60.mp3","https://healingofthequran.com/menu/2.mp3","generalroquia","الرقية الشرعية العامة","2"];
    ...
    downloadeRecordingAndroid(index:any){
        ...
        this.translate.get('generalroquia').subscribe( value => {
    ...
    playRecording() {
        this.storage.get('changeSound').then((val) => {
            this.redingType = val;
            this.formpagesProvider.watchList(this.soundOfArray[val][3],'2',15,this.soundOfArray[val][2],this.soundOfArray[val][1]).then(data=>{})
            for(let i=1;i<this.soundOfArray.length;i++){
...
//added to bokmark
    ...
    addBookMark(){
        ...
        this.bookmarksArray = [this.pageNameTra,15];
    ...
    removeBookMark(){
        this.bookMark().then(realVal=>{
            for(let i = 0;i < realVal.length;i++){
                realVal.splice(realVal.findIndex(elm => realVal[i][1] == 15) ,1)
            }
            this.imageType = 0;
            this.storage.set('bookMarks',realVal);
        });
